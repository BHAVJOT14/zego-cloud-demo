...
export default {
  ...
  methods: {
    ...
    async startPlayingStream(stream_Id) {
    ...
    async startCoHosting() {
      const localStream = this.createZegoExpressEngine();
      if (!localStream) {
        return;
      }

      try {
        await this.zg.startPublishingStream(localStream);
        console.log('Co-hosting started successfully');
      } catch (error) {
        console.error('Error starting co-hosting:', error);
      }
    },
